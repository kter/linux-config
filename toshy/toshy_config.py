...
throttle_delays(
    ...
    key_post_delay_ms   = 2,       # default: 0 ms, range: 0-150 ms, suggested: 1-100 ms
)
...
### 
### # 2. ショートカットの定義
### keymap("User Mac-like Cursor & System", {
###     # --- Control(元CapsLock)系: Mac風カーソル移動 ---
###     K("C-a"): K("home"),    # 行頭へ (Beginning of line)
###     K("C-e"): K("end"),     # 行末へ (End of line)
###     K("C-f"): K("right"),   # 右へ (Forward)
###     K("C-b"): K("left"),    # 左へ (Backward)
###     K("C-p"): K("up"),      # 上へ (Previous line)
###     K("C-n"): K("down"),    # 下へ (Next line)
###     K("C-d"): K("delete"),  # 1文字消去 (Delete)
###     K("C-h"): K("backspace"), # 1文字戻って消去 (Backspace)
### 
###     # --- IME切り替え ---
###     K("C-Space"): K("C-Space"),
### 
###     # --- Command(元Winキー)系: 一般的なショートカット ---
###     # Win + A で 全選択 (Linux/Windowsの本来の Control+A を送出)
###     K("RC-a"): K("C-a"),
###     # 必要に応じて Win + C (コピー) などもここに追加できます
###     # K("RC-c"): K("C-c"),
### })

## # 1. 物理的な入れ替え（システム全域でCapsLockをCtrlにする等のベース設定）
## define_modmap({
##     Key.CAPSLOCK: Key.LEFT_CTRL,    # CapsLock -> Control(左)
##     Key.LEFT_META: Key.RIGHT_CTRL,  # Win -> Control(右/Command役)
##     Key.LEFT_ALT: Key.LEFT_META,    # Alt -> Meta(Option役)
...
##     K("C-h"): K("backspace"),
## 
##     # --- Command(元Winキー)系: Macの標準ショートカットを再現 ---
##     K("RC-f"): K("C-f"),      # Cmd + F -> ページ内検索 (Find)
...
##     K("RC-r"): K("C-r"),      # Cmd + R -> 更新 (Reload)
## }, when = lambda ctx: not matchProps(clas=term_regex)(ctx))
## 
## # C. IME切り替え：全アプリ共通
...
term_regex = r"^(foot|gnome-terminal|gnome-shells|konsole|Alacritty|kitty|xfce4-terminal|com\.mitchellh\.ghostty|wezterm)$"

# IDE (VSCodeなど)：カーソル移動のみ除外、Command系は有効
ide_regex = r"^(code|Code|VSCodium|code-oss|antigravity)$"

# カーソル移動から除外するアプリの合体リスト（ターミナル + IDE）
cursor_exclude_regex = r"^(foot|gnome-terminal|gnome-shells|konsole|Alacritty|kitty|xfce4-terminal|com\.mitchellh\.ghostty|wezterm|code|Code|VSCodium|code-oss|antigravity)$"
...
keymap("User Command Shortcuts", {
    ...
    K("LCmd-z"): K("C-z"),    # Cmd + Z -> 元に戻す
    K("LCmd-s"): K("C-s"),    # Cmd + S -> 保存
    K("LCmd-t"): K("C-t"),    # Cmd + T -> 新しいタブ
